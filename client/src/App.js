...
class App extends React.Component {
  ...
  // apigUrl = "https://c3nxtws5v0.execute-api.ca-central-1.amazonaws.com/test";

  currentBook = () => this.state.books.find(book => book.book === this.state.book);

  loadBooks = switchActive => {
    fetch(`${this.state.apigUrl}/books`)
      .then(res => res.json())
      .then(
        result => {
          this.setState({
            isLoaded: true,
            books: [...result.filter(book => book.started)].sort((a, b) =>
              a.started < b.started ? 1 : -1
            )
          });
          if (switchActive) {
            const active = result.find(book => book.active);
            if(active) {
              this.loadWords(active["book"]);
            }
          } else {
            if (this.state.book) {
  ...
  loadWords = book => {
    ...
    fetch(`${this.state.apigUrl}/words/${book}`)
      .then(res => res.json())
      .then(
        result => {
          this.setState({
  ...
  bookRawCsvHref = book => `${this.state.apigUrl}/bookcsv/${book}`;

  bookStatCsvHref = book => `${this.state.apigUrl}/wordcsv/${book}`;

  lineBook = book =>
    `${book.book} ${book.active ? "*" : ""} [${
      book.started ? moment.utc(book.started).fromNow() : ""
  ...
  componentDidMount() {
    ...
    .then(res => res.json())
    .then(res => {
      this.setState({apigUrl: res.apigUrl, phone: res.phone});
    }).then(() => {
      this.loadBooks(true);
        ...
        <div className="App">
          <Navbar bg="light" variant="light">
            ...
            <Nav className="mr-auto">
                ...
                {[
                  ...new Set(
                    this.state.books.map(book =>
                      moment(book.started).format("YYYY-MM-DD")
                    )
                  )
                ].map(day => (
                  <Dropdown.Item
              ...
              <Nav.Link
                href={this.bookRawCsvHref(this.state.book)}
                target="_blank"
              >
                messages.csv
              </Nav.Link>
              <Nav.Link
                href={this.bookStatCsvHref(this.state.book)}
                target="_blank"
              >
                stats.csv
              </Nav.Link>
            ...
            <Nav className="mr-fill">{this.state.phone}</Nav>
          </Navbar>
